...
ClapTrap::ClapTrap(void) : _Name("Default"), _Hit_points(HEALTH_PTS), _Energy_points(ENERGY_PTS), _Attack_damage(ATTACK_PTS)
{
	std::cout << this->_Name << "\tConstructed" << std::endl;
	return;
}

ClapTrap::ClapTrap(const std::string name) : _Name(name), _Hit_points(HEALTH_PTS), _Energy_points(ENERGY_PTS), _Attack_damage(ATTACK_PTS)
{
	std::cout << this->_Name << "\tConstructed" << std::endl;
	return;
}
...
void ClapTrap::beRepaired(unsigned int amount)
{
	...
	if (amount > HEALTH_PTS - hp)
		repaire = HEALTH_PTS - hp;
	else
		repaire = amount;
